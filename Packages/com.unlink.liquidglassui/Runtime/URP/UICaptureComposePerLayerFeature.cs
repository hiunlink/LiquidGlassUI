...
using TagUtils = Unlink.LiquidGlassUI.UICaptureComposePerLayerTagUtils;
using PassPool = Unlink.LiquidGlassUI.UICaptureComposePerLayerPassPool;
...
namespace Unlink.LiquidGlassUI
{
    ...
    public class UICaptureComposePerLayerFeature : ScriptableRendererFeature
    {
        ...
        public class Settings
        {
            ...
            [Header("注入时机")]
            public RenderPassEvent injectEvent = RenderPassEvent.AfterRenderingOpaques;
        }
        ...
        public override void Create()
        {
            ...
            {
                settings.config = LiquidGlassSettingsLoader.LoadDefault();
            }

            // —— 材质只在这里构建一次；优先使用 Inspector 赋值，否则回退到内置 Shader
            _matMip  = settings.config.mipCompositeMat ?? new Material(Shader.Find("Hidden/UIBGCompositeStencilMip"));
            _matGauss= settings.config.gaussianMat ?? new Material(Shader.Find("Hidden/UIBG_GaussianSeparable"));
        ...
        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData data)
        {
            ...
            {
                firstGlobalTextureInfo = GetOrCreateGlobalTextureInfo(settings.config.globalTextureName,
                    settings.config.resolutionScale);
            }

            var useHDR = settings.config.useHDR;
